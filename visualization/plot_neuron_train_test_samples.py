...
def plot_neuron_train_test_samples(single_worm_dataset, neuron_idx, 
  ...
  batch_indices = np.random.choice(trainX.shape[0], n_ex, replace=False) # pick n examples
  ...
  batch_indices = np.random.choice(testX.shape[0], n_ex, replace=False) # pick n examples
  for _, batch_idx in enumerate(batch_indices):
    yshift = yshifts[_]
