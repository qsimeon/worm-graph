...
def Plot3D(D, columnList):
    ...
    X, Y, Z = D[columnList[0]], D[columnList[1]], D[columnList[2]]
...
def PlotCCM(libMeans, args):
    ...
    ax = libMeans.plot(
        "LibSize", [libMeans.columns[1], libMeans.columns[2]], title=title, linewidth=3
    )
...
def PlotObsPred_(df, args):
    ...
    title = (
        ...
        + str(args.Tp)
        + "  ρ="
        + str(round(stats["rho"], 2))
        + " RMSE="
        + str(round(stats["RMSE"], 2))
    )

    df.plot(df.columns[0], ["Observations", "Predictions"], title=title, linewidth=3)
