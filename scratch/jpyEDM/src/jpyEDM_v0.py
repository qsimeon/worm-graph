...
def Dashboard():
    ...
    lib = widgets.IntRangeSlider(
        value=[1, 500], min=1, max=1000, step=1, description="lib"
    )

    pred = widgets.IntRangeSlider(
        value=[501, 600], min=1, max=1000, step=1, description="pred"
    )
    ...
    tau = widgets.IntSlider(value=-1, min=-50, max=50, step=1, description="tau")

    knn = widgets.IntSlider(value=0, min=0, max=50, step=1, description="knn")

    E = widgets.IntSlider(value=3, min=0, max=50, step=1, description="E")

    maxE = widgets.IntSlider(value=10, min=5, max=50, step=1, description="maxE")

    Tp = widgets.IntSlider(value=1, min=-50, max=50, step=1, description="Tp")

    maxTp = widgets.IntSlider(value=10, min=5, max=50, step=1, description="maxTp")

    exclusionRadius = widgets.IntSlider(
        value=0, min=0, max=100, step=1, description="exclRad"
    )

    theta = widgets.FloatText(
        value=3, description="theta", layout=widgets.Layout(width="50%")
    )

    thetas = widgets.Text(
        value=args.thetas, description="thetas", layout=widgets.Layout(width="100%")
    )

    alpha = widgets.FloatText(
        value=0.05, description="alpha", layout=widgets.Layout(width="50%")
    )
    # Callback function on alpha to instantiate solver
    alpha.observe(onAlphaChange, names="value")

    l1_ratio = widgets.Text(
        value="0.05", description="l1_ratio", layout=widgets.Layout(width="90%")
    )
    # Callback function on l1_ratio to instantiate solver
    l1_ratio.observe(onL1_ratioChange, names="value")

    generateSteps = widgets.IntText(
        value=0, description="generate", layout=widgets.Layout(width="50%")
    )
    ...
    multiview = widgets.IntSlider(
        value=0, min=0, max=20, step=1, description="multiview"
    )

    libsize = widgets.Text(value="20 200 20", description="libsize")

    subsample = widgets.IntSlider(
        value=100, min=5, max=200, step=1, description="subsample"
    )

    seed = widgets.IntText(value=None, description="seed")

    maxLag = widgets.IntSlider(value=15, min=0, max=50, step=1, description="maxLag")

    MI_neighbors = widgets.IntSlider(
        value=10, min=2, max=100, step=1, description="MI_neighbors"
    )
    ...
    outputSmapFile = widgets.Text(
        value="", description="S-Map Output", style={"description_width": "initial"}
    )

    outputEmbed = widgets.Text(
        value="",
        description="Embed Output",
        indent=False,
    ...
    plot = widgets.Checkbox(
        value=True, description="plot", indent=False, layout=widgets.Layout(width="60%")
    )

    randomLib = widgets.Checkbox(
        value=True,
        description="randomLib",
        indent=False,
        layout=widgets.Layout(width="60%"),
    )

    trainLib = widgets.Checkbox(
        value=True,
        description="trainLib",
        indent=False,
        layout=widgets.Layout(width="60%"),
    )

    embedded = widgets.Checkbox(
        value=False,
    ...
    excludeTarget = widgets.Checkbox(
        value=False,
        description="excludeTarget",
        indent=False,
        layout=widgets.Layout(width="60%"),
    )

    running = widgets.Checkbox(
        value=False,
        description="Running",
        indent=False,
        layout=widgets.Layout(width="60%"),
...
def DataDashboard():
    ...
    right_box = widgets.VBox(
        [widgets.HBox([Widgets["fileImport"], Widgets["importButton"]])]
    )
...
def MutualInfoDashboard():
    ...
    # Organize widgets
    left_box = widgets.VBox([Widgets["method"], Widgets["columns"], Widgets["target"]])

    mid_box = widgets.VBox([Widgets["maxLag"], Widgets["tau"], Widgets["MI_neighbors"]])

    right_box = widgets.VBox(
        [Widgets["fileImport"], Widgets["plot"], Widgets["verbose"]]
    )
...
def EmbedDashboard():
    ...
    mid_box = widgets.VBox([Widgets["E"], Widgets["tau"], Widgets["plotSelect"]])

    right_box = widgets.VBox(
        [
            Widgets["fileImport"],  # Widgets['outputEmbed'],
            Widgets["plot"],
            Widgets["verbose"],
        ]
    )
