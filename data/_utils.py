...
class NeuralActivityDataset(torch.utils.data.Dataset):
    ...
    * Using NeuralActivityDataset will ensure that sequences are generated
    ...
    def __init__(
        ...
        # Verify that `self.__data_generator()` method worked as expected
        assert self.num_samples == len(
            self.data_samples
        ), "Wrong number of sequences generated by `self.__data_generator()`!"
    ...
    def __data_generator(self):
        ...
        The resulting samples may have overlapped time steps depending the length of the original data, the seqeunce
        length and the number samples that are requested. A simple back-of-the-envelope calculation shows that the
        the sequences will have overlapped time steps if `num_samples * seq_len > max_timesteps`.
