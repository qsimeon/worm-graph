...
class MapDataset(torch.utils.data.Dataset):
  ...
  def __data_generator(self):
    ...
    if self.seq_len is not None:
      ...
      seq_lens = range(1, T-self.tau+1) if self.increasing else range(T-self.tau, 0, -1)
    for L in seq_lens:
      # a batch contains all data of a certain length
      batch = []
      # iterate over all start indices
      start_range = range(0, T-L-self.tau+1) if not self.reverse else range(T-L-self.tau, -1, -1)
