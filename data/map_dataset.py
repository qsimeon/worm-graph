...
class MapDataset(torch.utils.data.Dataset):
  ...
  you generate is unique. We can also do minibatching with the 
  MapDataset by defining a `batch_sampler`.
  '''
  def __init__(self, D, neurons=None, tau=1, seq_len=None, size=float('inf'), 
    ...
    Args:
      ...
      size: int or inf, number of (input, target) data pairs to generate.
              if inf, will generate all possible samples.
    ...
    Returns:
      (X, Y, metadata): tuple, btach of data samples
        ...
        metadata: dict, dictionary with information about samples, keys: 'seq_len', 'tau' offset, 
