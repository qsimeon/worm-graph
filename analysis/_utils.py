...
def plot_loss_vs_parameter(configs, param_names):
    ...
    for param_name in param_names:
        param_values[param_name] = set(
            get_config_value(config, param_name) for config in configs.values()
        )
    ...
    for param_values_combination in itertools.product(*param_values.values()):
        ...
        for config_path, config in configs.items():
            if all(
                get_config_value(config, param_name) == param_value
                for param_name, param_value in zip(
                    param_names, param_values_combination
                )
            ):
        ...
        for config_path, _ in matching_configs.items():
            ...
            if os.path.exists(loss_path):
                ...
                num_worms = (
                    len(os.listdir(config_path)) - 3
                )  # Subtract 3 for .hydra, config.yaml, and loss_curves.csv
        ...
        trailing_averages[param_values_tuple] = sum(trailing_avg_values) / len(
            trailing_avg_values
        )
    ...
    plt.title(
        "Mean Trailing Avg. Validation Loss vs. {}".format(", ".join(param_names))
    )
    param_values_tuples = list(trailing_averages.keys())
    param_values_lists = [list(t) for t in param_values_tuples]
    sns.regplot(
        x=param_values_lists,
        y=list(trailing_averages.values()),
        scatter=True,
        ci=95,
        order=2,
        label="Data",
    ...
    plt.savefig(
        os.path.join(
            os.getcwd() + "/analysis/figures/",
            "val_loss_on_[" + str(time_stamp) + "].png",
        )
    )
    plt.show()
...
def plot_loss_vs_parameter_sorted(configs, param_names):
    ...
    param_values_tuples = list(trailing_averages.keys())
    param_values_lists = [list(t) for t in param_values_tuples]
    sorted_x, sorted_y = zip(
        *sorted(zip(param_values_lists, list(trailing_averages.values())))
    )
...
def neurons_on_category(model, dataset, smooth_data, tau_out):
    ...
    for i in range(len(dataset)):
        ...
        for k in range(len(named_neuron_inds)):
            named_neuron_category_loss.setdefault(
                (named_neuron_inds[k], loss_prediction[k]), category_neuron[k]
            )
    ...
    plt.title(
        "Analysis on neuron-type and prediction-loss \n Dataset: "
        + dataset["worm0"]["dataset"]
    )
    plt.savefig(
        os.path.join(
            ROOT_DIR,
...
def plot_trailing_loss_vs_parameter_legend(config_pardir, parameter, legend):
    ...
    for para_n, val_dict in trailing_dict.items():
        sorted_x, sorted_y = zip(
            *sorted(zip(list(val_dict.keys()), list(val_dict.values())))
        )
        x = sorted_x
    ...
    plt.title(
        "Analysis on "
        + str(param_name)
        + " \n Trailing Losses for Different "
        + str(legends[1])
        + "s"
    )
    plt.savefig(
        "analysis/figures/trailing_loss_on_"
        + str(parameter)
        + "_legend_"
        + str(legend)
        + ".png"
