...
class OneStepPrediction(GraphTask):
    ...
    def __init__(self, graph, dataset, neuron_ids):
        ...
        assert (
            dataset.ndim == 2
            and dataset.shape[1] < dataset.shape[0]
            and dataset.shape[1] <= self.graph.num_nodes
        ), "Reshape neural data as (time, neurons)!"
        assert len(neuron_ids) <= dataset.shape[1]
        # find the graph nodes matching the neurons in the dataset
        graph_inds = [
            k - 1 for k, v in graph.id_neuron.items() if v in set(neuron_ids.values())
        ]  # neuron indices in connectome
        data_inds = [
            k_ - 1
            for k_, v_ in neuron_ids.items()
            if v_ in set(graph.id_neuron.values())
        ...
        loader = torch.utils.data.DataLoader(
            __, batch_sampler=BatchSampler(__.batch_indices)
        )
        ...
        self.temporal_dataset = StaticGraphTemporalSignal(
            edge_index, edge_weight, features, targets
        )
        # split into train and test sets
        self.train_dataset, self.test_dataset = temporal_signal_split(
            self.temporal_dataset, train_ratio=0.5
        )
